...
-- Adds the login and patch servers' live settings: settings holds the value a setting was given while the server ran, which outranks its config value until it is reset, and setting_audit keeps one row for every change with the value before and after, who made it, where it came in and why.
