...
>
    {#snippet actions()}
        <Button onclick={() => void save()} disabled={busy || answer === null}>
            <SaveIcon />{busy ? "Saving…" : "Save changes"}
        </Button>
    {/snippet}
</PageHeader>
...
{/if}
{#if notice}
    <p class="rounded-md border border-healthy/30 bg-healthy/5 p-3 text-sm text-healthy" role="status">
        {notice}
    </p>
{/if}
                        ...
                        <div class="space-y-2">
                            ...
                            {#if setting.secret}
                                <p class="text-xs text-muted-foreground">
                                    Stored secret is never returned. Leave blank to keep it.
                                </p>
                            {/if}
                        </div>
                    {/each}
