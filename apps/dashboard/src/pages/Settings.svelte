                    ...
                    {#each shown(group.id) as setting (setting.key)}
                        <div class="space-y-2">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                {/if}
                                {#if setting.locked}<StatusBadge tone="unknown">Locked · {setting.layer}</StatusBadge>{/if}
                            </div>
                            {#if setting.secret}<p class="text-xs text-muted-foreground">Stored secret is never returned. Leave blank to keep it.</p>{/if}
                        </div>
                    {/each}
                </Card.Content>
                {#if group.id === "mail"}<Card.Footer><Button variant="outline" onclick={() => void sendTest()}><MailIcon />Send test to me</Button></Card.Footer>{/if}
            </Card.Root>
        </Tabs.Content>
    {/each}
</Tabs.Root>
