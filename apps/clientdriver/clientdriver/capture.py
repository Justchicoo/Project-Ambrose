...
# Captures the run's loopback traffic with tshark and finalizes the file the only way tshark does, a console control event on its own hidden console, so the capture is readable even when a run fails.
import os
import re
import subprocess
import sys
...
class Capture:
    ...
    def start(self, timeout=30):
        ...
        try:
            ...
            self.note = f"tshark did not start: {error}: {' '.join(tail.lines)}"
            self.process = None
            return self.note
    ...
    def stop(self):
        ...
        try:
            ...
            how = "a console control event"
        except subprocess.TimeoutExpired:
            from .netguard import kill_tree

            kill_tree(self.process.pid)
            how = "a kill, so the capture may be cut short"
        if self._errors:
            self._errors.close()
