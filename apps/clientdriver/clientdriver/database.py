    ...
    def seed_character(self, user, wizard):
        ...
        try:
            with connection.cursor() as cursor:
                ...
                cursor.execute("INSERT INTO characters (guid, account, name_indices, school_id, level, world, zone, zone_display, created) "
                               "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, UNIX_TIMESTAMP())",
                               (guid, account, indices, int(wizard["school"]), int(wizard.get("level", 1)), int(wizard.get("world", 0)),
                                wizard["zone"], wizard.get("zone_display", "")))
    ...
    def read_wizard(info, guid):
        ...
        try:
            with connection.cursor(pymysql.cursors.DictCursor) as cursor:
                cursor.execute("SELECT name_indices, school_id, level, world, zone, zone_display FROM characters WHERE guid = %s", (int(guid),))
                row = cursor.fetchone()
                if row is None:
            ...
            connection.close()
        return Scratch.wizard_from_rows(row, look)

    @staticmethod
    def wizard_from_rows(row, look):
        indices = int(row["name_indices"])
        locale = (indices >> 24) & 0xFF
        return {"school": int(row["school_id"]), "level": int(row["level"]), "world": int(row["world"]), "zone": row["zone"],
