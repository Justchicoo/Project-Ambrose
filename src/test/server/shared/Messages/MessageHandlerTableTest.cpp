...
{
    TestTable rest;
    rest.PendingRest(7, SessionStatuses::Any);
    std::vector<std::string> errors;
    EXPECT_TRUE(rest.Validate(*_catalog, errors)) << (errors.empty() ? std::string() : errors.front());

    CapturedLog log;
    DmlMessageData unlisted = Message(7, 5);
    EXPECT_EQ(rest.Dispatch(_session, _catalog, unlisted), DispatchResult::NotHandled)
        << "a message only the rest rule covers is one this app does not handle yet, not one it has never heard of";
    EXPECT_TRUE(log.Contains("LOGIN MSG_UNLISTED (7:5), which testserver does not handle yet"));
    EXPECT_TRUE(_session.Strikes.empty()) << "and it costs the client nothing, because the client did nothing wrong";

    DmlMessageData reply = Message(7, 3);
    EXPECT_EQ(rest.Dispatch(_session, _catalog, reply), DispatchResult::Refused)
        << "a message the table names itself keeps its own rule rather than falling to the rest";
    EXPECT_TRUE(log.Contains("LOGIN MSG_REPLY (7:3), which only the server sends"));

    DmlMessageData foreign = Message(5, 1, { 0, 0 });
    EXPECT_EQ(rest.Dispatch(_session, _catalog, foreign), DispatchResult::Refused)
        << "a rest rule covers its own service and no other";
    EXPECT_TRUE(log.Contains("GAME MSG_JOIN (5:1), which testserver never accepts"));

    HelloMessage greeting;
    greeting.Version = "W.1.610";
    greeting.Machine = 77;
    DmlMessageData hello = Encoded(*_catalog, greeting, 1);
    _session.Status = SessionStatus::Connected;
