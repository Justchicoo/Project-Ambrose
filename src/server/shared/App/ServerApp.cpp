    ...
    : _info(std::move(info)), _category("server." + _info.Name), _config(config), _log(log), _out(out), _err(err), _updateTimer(_io.GetImpl()), _shutdownTimer(_io.GetImpl())
{
    ...
    _commands.Register({ "reload", "[target|all]", "list what can be rebuilt without a restart, or rebuild one by name, or all", true,
        [](std::vector<std::string> const& arguments, ConsoleCommandTable::Reply const& reply)
        {
            if (arguments.size() > 1)
                return false;
            auto say = [&reply](std::vector<std::string> const& lines)
            {
                for (std::string const& line : lines)
                    reply(line);
            };
            if (arguments.empty())
            {
                say(sReloadMgr.DescribeTargets());
                return true;
            }
            if (arguments.front() == "all")
            {
                ...
                for (ReloadOutcome const& outcome : outcomes)
                    say(ReloadMgr::Describe(outcome));
                return true;
            }
            ...
            say(ReloadMgr::Describe(sReloadMgr.Reload(arguments.front())));
            return true;
        } });
