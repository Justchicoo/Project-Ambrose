 ...
 * What client discovery may ask of the machine it runs on: environment variables, files and folders, the canonical form of a path with its links resolved, the Windows uninstall entries and the Steam install path, behind an interface so tests can describe a machine; LocalClientSystem answers for the real one.
 */
...
public:
    ...
    virtual std::filesystem::path Canonical(std::filesystem::path const& path) const = 0;
