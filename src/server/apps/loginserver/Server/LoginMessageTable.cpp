 ...
 * Lists every message of the login server's services once: MSG_USER_AUTHEN_V3 and the older authentication messages handled before authentication, MSG_REQUESTCHARACTERLIST, MSG_REQUESTSERVERLIST, MSG_SELECTCHARACTER, MSG_CREATECHARACTER and MSG_LOGINLOGCHARACTERCREATION handled once authenticated, MSG_LOGIN_NOT_AFK handled from connection through character selection, the other LOGIN client requests with the status each will need, the server's replies refused and the ones it sends declared, and the SYSTEM and EXTENDEDBASE rules every app shares.
 */
