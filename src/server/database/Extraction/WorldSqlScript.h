 ...
 * A script of world database statements that replaces whole tables with extracted rows, whose text values travel as hex literals so no sql_mode or escaping can change them; it renders as one transaction written whole to a file or not at all, commits its statements in one transaction on a connection and says when a lost connection leaves the commit's outcome unknown, and can check that a database holds the tables it would write.
 */
...
{
public:
    using Value = std::variant<uint64, std::string>;
