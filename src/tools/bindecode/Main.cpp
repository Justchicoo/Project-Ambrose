...
with --raw an entry that holds a versionable object but no BINd header, and with
--text any other entry, such as one of the client's own XML files, as the
text it holds.
...
Options:
  ...
  --raw               read an entry as a versionable object stored with no BINd header,
                      as a zone's gamedata.bin is
    ...
    int PrintEntries(Arguments const& arguments, KiwadArchive const& archive, TypeCatalogPtr const& catalog)
    {
        ...
        for (std::string const& name : arguments.Entries)
        {
            ...
            if (arguments.Raw)
            {
                ...
                decoded = ObjectSerializer::Decode(catalog, read.Data, options);
                if (!decoded.Ok())
                {
                    std::cerr << fmt::format("{}: does not hold a versionable object: {}\n", name, decoded.Detail);
                    status = Failure;
                    continue;
                }
            }
            ...
            {
                BindReadResult result = BindFile::Read(catalog, read.Data);
                if (!result.Ok())
                {
                    std::cerr << fmt::format("{}: {}: {}{}\n", name, BindFile::GetStatusName(result.Status), result.Detail,
                        result.Status == BindStatus::NotBind ? "; an entry that holds a versionable object with no BINd header, such as a zone's gamedata.bin, is read with --raw" : "");
                    status = Failure;
                    continue;
                }
                decoded = std::move(result.Decoded);
            }
            ...
            std::cout << PropertyJson::Dump(decoded.Object.get(), arguments.Compact ? -1 : 2) << '\n';
        }
        return status;
    }
