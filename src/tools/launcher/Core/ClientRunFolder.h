 ...
 * The folder an Ambrose-started client runs from, built beside the install and never inside it: config.xml and preferences.xml written every run, from the files the folder already holds so the client's own saved settings survive, or from the install's own files, or from defaultconfig.xml in its Root.wad when it has no config.xml, each a copy of its template with only the window mode, size and position asked for changed and SilentMetricsURL emptied so the client reaches nothing outside the machine, and copies of revision.dat and data.dat, the other files the client opens by relative name. A stamp records the install and revision those copies were made from, so they are made again when either changes and the two generated files are seeded from the install again with them.
 */
