...
- Phase 4 is under way, so check before assuming: the world tick, `ScriptMgr` and the script and module loader are in, `CommandMgr` and its security levels are in `src/server/game/Chat`, the realm registry and heartbeat are in, movement packing and the location string are in, character select hands the client to the game server, a zone extractor reads every zone in an installation into world rows, `sZoneMgr` holds every zone template, place and object placement, and `sMapMgr` runs zone instances. The game server loads an attaching wizard, builds its object from the player template the install's TemplateManifest.xml names, and sends MSG_LOGINCOMPLETE. Phase 5 has begun: `extractor levels` and the game server's first start fill the level, school and stat tables from my install (5.04), and a wizard enters the world with its level, experience, health, mana, gold and training points from the database, written back when they change and when it leaves (5.05).
- **These do not exist yet**, and a milestone that needs one is blocked rather than open: the full template store (5.01), objects spawned into a zone, movement, other players, and everything gameplay after them.
