...
- Phase 4 is under way, so check before assuming: the world tick, `ScriptMgr` and the script and module loader are in, `CommandMgr` and its security levels are in `src/server/game/Chat`, the realm registry and heartbeat are in, movement packing and the location string are in, character select and the handoff to the game server are part built, and a zone extractor reads every zone in an installation into world rows.
- **These do not exist yet**, and a milestone that needs one is blocked rather than open: the zone manager, the template store, and everything gameplay after them.
