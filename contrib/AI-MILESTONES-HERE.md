...
- Phases 1 to 3 are built, except for a few checks that need a real client at the keyboard, and phase 4 is under way. A real client started by Ambrose's own launcher signs in, lists and creates wizards, and through the client driver's `enter-world.json` it enters the world and stands in Ravenwood, and when it walks, the server keeps where it stands and brings it back to that spot at the next login. Nothing else in the world answers it yet: there are no other objects, no NPCs, chat, quests, pets, housing or combat, on either side.
- The pinned client revision is r806919 (Wizard 1.610). Read mine from `Bin/revision.dat` in my own install rather than copying an example. A Steam installation works and is what the maintainer uses; its path has spaces in it, so quote it everywhere.
- **These exist. Do not build them again.** `src/common/` has configuration, logging, threading, cryptography, encoding and utilities. `src/server/shared/Archives` reads KIWAD. `src/server/shared/Messages` loads the client's own message XML at run time into a registry. `src/server/shared/ObjectProperty` encodes and decodes both the compact wire format and the versionable BINd format, with a type registry, property enums, defaults, a JSON view and a fuzz test. `src/server/shared/ClientData` finds the install, keeps a per-revision type dump and extracts name tables. `src/server/shared/Network` has frames, the socket layer and the session handshake. `src/server/database` has the connection pool, the dated SQL updater and extraction. `src/server/game` has the world tick that owns the world thread, `ScriptMgr` and its hook classes, and `GameSession`, whose queue drains on that thread; a script or a module joins the build by existing, because CMake writes the loader from every `AddSC_` in `src/server/scripts` and every module under `modules/`. `ScriptLoader.h` lives in `src/server/scripts`, not in the scripting library, and `ScriptMgr::LoadScripts` takes the loader as an argument rather than reaching for it, so the hooks never depend on the content that uses them. `src/server/shared/Admin` serves each app's admin API, and `src/common/Metric` is the metrics registry every app answers `GET /metrics` from. The supervisor runs the apps and serves the panel, `apps/dashboard`, on a listener of its own with TLS, sessions, an audit store, panel accounts and a permission catalog with roles and grants; it also samples every app into the resource graphs. `apps/grafana` is a provisioned Prometheus and Grafana stack, and `apps/installer` takes a checkout to running servers. `src/tools` has `client`, the one tool that asks an install a question and the first to look at, which also reads the client program's own code through `strings`, `xrefs`, `disasm`, `functions`, `vtable` and, with my own Ghidra, `decompile`, plus `typeextract`, `bindecode`, `schemaprobe`, `localetool`, `extractor`, `zone_extractor`, `dbimport`, `launcher`, `patchlist_generator` and `typeregbuild`.
- Phase 4 is under way, so check before assuming: the world tick, `ScriptMgr` and the script and module loader are in, `CommandMgr` and its security levels are in `src/server/game/Chat`, the realm registry and heartbeat are in, movement packing and the location string are in, character select hands the client to the game server, a zone extractor reads every zone in an installation into world rows, `sZoneMgr` holds every zone template, place and object placement, and `sMapMgr` runs zone instances. The game server loads an attaching wizard, builds its object from the player template the install's TemplateManifest.xml names, and sends MSG_LOGINCOMPLETE. Phase 5 has begun: `extractor levels` and the game server's first start fill the level, school and stat tables from my install (5.04), and a wizard enters the world with its level, experience, health, mana, gold and training points from the database, written back when they change and when it leaves (5.05). The server tracks where the wizard stands from the moves its client sends, read with the client's own packing, and saves the place when it leaves the world (5.03). `sObjectTemplateMgr` reads the install's TemplateManifest.xml and gives any template by id, decoded from my WADs the first time it is asked for and kept, and `.reload templates` swaps in an edited manifest while a broken one keeps the old (5.01). It also reads every spell and sigil template my install holds into typed records and finds one by the client's hash of its name, as the client does (8.04, 9.02). A wizard's spellbook travels in its own player object at login, and `learn` and `unlearn` change it live and keep it across a relog (8.05).
- **These do not exist yet**, and a milestone that needs one is blocked rather than open: objects spawned into a zone, other players and their movement, and everything gameplay after them.
...
**The admin API answers only host names it knows.** A request whose `Host` header it does not recognise gets `400 host_not_allowed`, which guards against DNS rebinding. A container that reaches it as `host.docker.internal` needs that name in `Admin.AllowedHosts`, as doc/OPERATIONS.md says. The refusal names its reason, so read the body before suspecting the token.
