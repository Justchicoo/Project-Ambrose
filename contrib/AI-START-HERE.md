...
- Phases 1 to 3 are built, except for a few checks that need a real client at the keyboard, and phase 4, a wizard standing in Ravenwood, is under way. A real client started by Ambrose's own launcher signs in, lists and creates wizards, and through `apps/clientdriver/scenarios/enter-world.json` enters the world and stands in Ravenwood. Nothing in the world answers it yet: no other objects, NPCs, movement, chat, quests, pets, housing or combat, on either side.
...
- `src/tools/client` is the one tool that asks my own install a question, and the first place to look: `client types <name or hash>` prints a class from the type dump with its bases, properties, flags, ids and enum options, which is the only way to read a dump at all because it is keyed by hash; `client messages <tag>` prints what the client says a message carries, from its own definitions; `client wad <entry>` prints an archive entry, BINd as JSON; `client core` and `client hex` read a game object blob and a file's bytes. Each takes `--list` for a wide match.
- The other built tools in `src/tools/`: `typeextract` (builds my own client's type dump by emulating its program, without launching the game; takes `--client` and `--out`, and finds the install from `AMBROSE_CLIENT_DIR` or the newest revision), `bindecode` (prints the named BINd entries of one KIWAD archive of my own install as JSON; also `--list <pattern>` and `--sweep`; it reads an archive, not a capture, and has no `--blob` flag and no standard input), `schemaprobe` (sweeps Root.wad or every GameData archive for classes and properties the dump does not name, with counts and paths), `zone_extractor` (reads every zone in an install into world rows), `localetool` (the `.lang` text of my own Root.wad), `extractor` (name tables into a world database), `dbimport` (creates the databases a server needs), `launcher` (starts my own client against an Ambrose login server without ever running KingsIsle's launcher), `patchlist_generator` (a patch manifest from my own install) and `typeregbuild` (a binary cache of a type dump). doc/TOOLS.md says which of them are still in flight and what each does not do yet.
- Seventy-one contributor items are merged, listed with their folders at the end of doc/CONTRIBUTOR-TRACK.md: tools, schemas, fixtures, audits, guides, proposals and scenarios. Read the ones nearest my item before building anything similar, both to avoid redoing them and to see the standard.
- `apps/clientdriver` drives my own client through a scenario with nobody at the keyboard. `python apps/clientdriver/drive.py check` says in one line whether my machine can run it and exits 77 when it cannot; have me run that before planning anything that needs it. It requires Windows, `AMBROSE_CLIENT_DIR` or `--client`, the built server and launcher, a reachable MySQL or MariaDB, `tshark` with an Npcap loopback adapter, the pinned packages in `apps/clientdriver/requirements.txt`, and reference crops rebuilt by `drive.py capture-refs` at my own revision, window size and interface scale, because crops are pictures of the client and are never committed. Presses are measured at 1280x720 on r806919.
- A scenario is data: steps that wait on a line of the login server's log (`wait_server_log`), the game server's (`wait_game_log`), the client's (`wait_client_log`), a screen or a database row, plus `server_command` for the login server's console, `server_settings` and `game_settings` for their options, `requires.gameserver` for a scratch game server, and `wizard` to seed the run's one wizard. It may name only screens already in `apps/clientdriver/references.json`: `login`, `invalid`, `charselect`, `dialog_next`, `dialog_back`, `test_book`, `school_list`, `school_chosen`, `appearance`, `name`. That file and the driver's own code are outside the track and are being changed for the world right now, so a scenario needing a new screen or a new action has to be a proposal instead, and a scenario that goes into the world includes `enter-world.json` rather than copying its steps.
...
- My own install and nothing built: the client and revision findings that need no tool, and tools of my own that read an installation.
- Nothing but thought: the proposals.
- My own install and a C++ build of the tools: the findings in the third and second lists, which `client`, `bindecode` and `schemaprobe` answer.
- A full C++ build: anything that runs the servers or their tools.
- MySQL 8 and MariaDB side by side, which Docker gives in a minute: the SQL corpus and its checker.
...
**Start from the third list.** Every row in it names the milestone it takes work off, so a merged item is time taken off the roadmap: scenarios that turn a milestone's real-client check into one command the maintainer runs, answers to the questions the phase files mark unverified, read from my own install with the built tools, and corpora a milestone's tests will load. Before taking a row, read that milestone's section in its phase file, because the row quotes it and the milestone is what proves the answer. Each list carries a contract in the track's own section, needs, delivers, proven by and refused, and the refused line is what a review will hold me to. The second list's findings, F-41 to F-55, need only my install and the built tools. The first-list findings that need a running combat or world server cannot be done yet, because those servers do not exist; do not attempt them and do not record that they cannot be done.
