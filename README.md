...
Pre-alpha. The build toolchain, unit tests, style checker, and CI on Windows and Linux are in place. A real Wizard101 client, started by Ambrose's own launcher, now talks to the servers as far as character select: the session handshake, signing in, a wrong password and a retry, an empty character list and the shutdown notice have all been watched working against a retail client. It stops there. No world, quest or combat traffic appears yet, and [doc/ROADMAP.md](doc/ROADMAP.md)'s "Where we are" says exactly which milestones are done.
