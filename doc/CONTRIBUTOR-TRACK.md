...
- **Scenarios that turn a real-client check into one command.** Most of the roadmap waits on a few early milestones whose code is finished and whose last checks need a real client at the keyboard. 2.09's last check alone stands between the project and 9.01, the combat service's message surface. A scenario the client driver runs makes each of those checks a command the maintainer runs in a minute. The scenario format is stable, and these stay on the login and character screens, which nothing being built touches.
...
|---|---|---|---|
| C-76 | Scenario for 2.09's last check: sign in with the login server's authentication log at debug, and prove MSG_USER_AUTHEN_V3 arrives decoded field by field by waiting for the line that logs its version, revision, locale and machine | `apps/clientdriver/scenarios/` | 2.09 is finished but for this check, and 4.12 and 9.01 both wait on 2.09 |
...
| F-56 | For 5.04: whether MagicXPConfig.xml's `m_classInfo` is keyed by school name and how its `m_levelInfo` list is laid out, with class names, field names, types and level counts | `contrib/findings/data/` | 5.04's spec marks exactly this unverified, and the level, school and stat extractor is built on it |
| F-57 | For 5.05: every property of WizGameStats with its type and flags from the dump, and which files in the install set any of them | `contrib/findings/objects/` | 5.05 fills these for every wizard, and its spec says nobody knows which ones the client needs set |
