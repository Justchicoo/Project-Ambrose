...
| JSON | nlohmann-json |
| Compression | zlib |
| Cryptography | Botan 3, covering SHA-2, Twofish, Argon2id for panel passwords, AES-256-GCM, HMAC-SHA-256, HKDF-SHA-256, Ed25519 and the random number generator. The client's non-standard CRC-32 is implemented in `common` |
...
- Dispatch follows AzerothCore's split between messages the server never accepts and messages it does not handle yet. A message listed as refused, a message from a service the app does not serve, an id the definitions do not have, a body shorter than its definition, or a MSG_PING beyond the session's ping budget counts a strike, and `Network.MaxStrikes` strikes close the session. A message in the wrong status, or one not handled yet, is dropped and logged without a strike, so a real client is not disconnected for sending something Ambrose has not implemented. A message not handled yet is logged only the first time a session sends it and counted every time after, because a real client repeats many of them, fifty MSG_REQASKSERVER in its first second in the world, and the milestone that answers one needs to know it is sent, not how often. Each session may drop only `Network.DroppedMessageBurst` messages, refilled at `Network.DroppedMessagesPerSecond`, while every drop is logged; beyond that a drop is not logged and counts a strike, so one client can neither fill the logs nor stay connected by flooding. A repeated message not handled yet spends none of that budget, and a new one spends it as any drop does, so a client naming every message it knows still strikes out. Client-supplied text is escaped and cut to 64 bytes before it reaches a log line.
...
- `auto` never asks. When `ClientDir` is empty, a server uses the install with the newest revision, preferring one with the client program when revisions tie and then the one whose Data/GameData holds more archives, because a download that streams the rest of the game on demand holds fewer than one that has it all, saves it to `conf.d/client-data.conf` beside the configuration, and the configuration reloads. When `ClientDir` is set but holds no install, for example on a drive that is not mounted, the newest install is used for that run only, the setting stays as it is, and a warning names the file to edit. When `TypeDumpPath` is empty or not a type dump, it uses the dump built from the install in use, and that path is not saved. A `TypeDumpPath` the command line sets empty stays empty, and no dump is built for it.
...
- `server_class`, `server_class_base` and `server_class_property` hold the classes the client uses that its dump does not describe, in the dump's own terms: a name and its hash, its bases in order, and its properties with their ids, types, hashes, offsets, flags and containers. `sObjectSchemaMgr` reads them in one snapshot into the dump's raw shape and hands them to `sTypeRegistry`, which checks every name and property against the hash formula the client uses and builds them into the catalog beside the dump's classes; a class the dump describes itself is taken from the dump. The registry keeps the dump each catalog was built from, so `.reload server_class_schema` rebuilds the next generation without reading the dump again, and a failure keeps the catalog that was serving.
- `core_object_type` holds the block and type each game object class is created with, and `behavior_client_class` the class the client builds for each behavior a template names, NULL where the client takes that slot empty. Both are checked against the catalog in use before they are swapped in: every class must be listed, a game object's must derive from CoreObject and a behavior's from BehaviorInstance, no pair may repeat or be the plain pair 0 0, and no class may hold two pairs. They reload as `core_object_type` and `behavior_client_class`, after `server_class_schema`.
- Every row carries the evidence that proves it, and a row is added only when a capture or the client itself proves it. Class, property and behavior names are client identifiers, which authored SQL may hold.
- The servers open their databases before they load the type dump, so these classes join its first build rather than a second one. The client tool reads the same tables when it is given the world database.

### Server-side content

Settled on 2026-09-25 under the maintainer's standing delegation. The maintainer's own CSR package, a CSR.wad that older clients shipped and current installs no longer carry, showed how KingsIsle's server-side definitions refer to the client: its quest and badge templates hold locale keys, persona and template names, requirements, image and sound paths and numbers, and no display text. Content comes from three sources, and every installation has the first two.

- The user's own install, read automatically like the rest of its data. It holds more than was expected: every quest title, goal and line of dialogue and every badge name in its locale files, each level's experience, health, mana, gold cap and pip chance in `MagicXPConfig.xml`, canonical stats for each school and level in `LevelScaledData.xml`, the badge filters, daily quest data, the battleground, polymorph and weaving decks, and the templates, zones and triggers content points at. A milestone that needs content first asks the client tool in src/tools/client what the install holds, and reads what it finds there rather than authoring it.
- Authored world SQL for what no current install carries: who offers each quest, its goals and counts and what follows it, the decks of ordinary creatures, drop tables, vendor stock, trainer lists, what earns each badge, bank capacities and potion rates. Rows hold client identifiers and numbers and never display text, under the rule in Experimental features, so each install supplies the words at runtime. Contributors write them from what they observe in play and from the client's own data, each row with the evidence it rests on, and they are committed, so every installation has them.
- Opt-in importers for files a user holds that others lack, such as a CSR package kept from an older client or another project's data set. They write only that user's local world database, every imported row names its source, and no export into data/sql takes such a row. Nothing read from such a source is committed, in bulk or copied by hand, because it is data the project does not own. 7.05 imports a CSR package's quests and 12.08 its badges.
...
Settled on 2026-09-25 at the maintainer's direction to settle every open decision. Each row replaces a proposal doc/PANEL.md made and doc/ROADMAP.md listed under Decisions needed.

| Area | Choice |
|---|---|
| Command level on the panel console | A `console.write` grant carries the highest CommandMgr security level its holder may run at: game master (2) in the operator and game master roles, console (4) only in the owner and admin roles. A panel user linked to a game account runs at the lower of the grant's level and that account's level, so a link can lower the ceiling and never raise it; an unlinked user runs at the grant's. The supervisor passes that level on every relayed command, a command above it answers as unknown as 4.02 does, and typed actions such as ban, kick and password reset check their own permission and the level of the command they run. How a level reaches LOGINCOMPLETE is under Entering the world |
| Tick profiles | No profiler library. The tick's own subsystem timers from 17.91, which cost one relaxed flag check while no capture runs, record begin and end events into a buffer bounded by the capture's seconds and an event count, and write them as a Chrome trace event file in JSON, which Perfetto's trace viewer and Chrome's own tracing page read. Nothing opens a listener: Tracy was set aside because its viewer connects to a port the profiled program opens, which 17.91's own check forbids. Sampling native stacks stays with the operating system's own profilers |
| Scopes and default roles | Grants apply at five scopes, widest first: panel, node, cluster, realm and app, and a grant at one covers everything below it. The permission catalog's Scopes column says where each key may be granted, a user's effective permissions at a scope are their role plus every grant at that scope and its ancestors, and an API key is intersected with that set on every request. The five default roles are the ones 17.48 built, owner, admin, operator, game master and viewer, as doc/PANEL.md lists them, and the owner-only set is the one it names. Until 17.22 and 17.31 build nodes, clusters and realms, app scope is the only one a grant can name |
| Keys the supervisor holds | A keyring file separate from the store, `keyring` in the Ambrose data folder, so a copy of the store alone, such as a backup's `panel` component, never carries the keys that open its secrets. Each key has an id and the rows it sealed carry that id, so keys rotate the way `Account.VerifierKeys` does. On Linux the file is owned by the service user with mode 0600 in a 0700 folder; on Windows its access list names only the service account, SYSTEM and Administrators, and its contents are wrapped with DPAPI for that account. In Docker it lives on the data volume. Exporting a key is owner-only and needs a step-up check |
| Sealing and keyed hashes | Botan, with no second library: AES-256-GCM seals every secret the supervisor must use again, HMAC-SHA-256 keys every hash that must be found by lookup, such as recovery codes and API key secrets, and HKDF-SHA-256 derives a separate key per purpose from a keyring key. Stack's cryptography row names all three |
| Outbound HTTP | libcurl, over the OpenSSL that already serves TLS inbound, so one TLS library covers both directions. It is added with the first milestone that makes an outbound request: S3 storage, alert webhooks, update channels, remote pull or certificate issuance |
| Certificates for a hostname | The panel implements ACME (RFC 8555) itself: an ECDSA P-256 account key in the keyring, JWS signing and the certificate request through Botan, the HTTP-01 challenge answered on port 80 when the operator turns it on, renewal at two thirds of the lifetime, and the new certificate swapped into the listener live the way a reload already swaps one. Running a reverse proxy that obtains certificates itself stays documented as the other path |
| Event socket protocol | As Event socket in doc/PANEL.md describes it: one WebSocket with a versioned JSON envelope whose fields are only ever added, the client and server message types listed there, close codes 4400, 4401, 4403 and 4429, a `hello` carrying the CSRF token or a single-use ticket for API keys, a ping every 20 seconds with a close after 60 silent seconds, and TypeScript types generated from the C++ schemas with a contract test |
| Backup archives | Sealed by default, not opt-in, because an archive holds account verifiers and configuration secrets: AES-256-GCM under a backup key in the keyring, whose export an owner is asked for until they have taken it, since an archive without its key cannot be restored anywhere else. Databases are dumped as structured rows, never SQL text: each table's column names and types, then its rows in primary-key order as JSON lines with binary values in base64, compressed with the archive, and restored through prepared inserts in batches, so a restore never parses SQL out of a file |
| S3-compatible storage | Signature Version 4 implemented over Botan's HMAC-SHA-256 and SHA-256 and sent through the outbound HTTP client, with path-style and virtual-hosted addressing and multipart uploads. aws-sdk-cpp was set aside for its size and build time when the panel needs only one service's small part of it |
| Login-screen countdowns | Earlier warnings go to players in the world, and a player on the login screen gets only the final shutdown notice, because MSG_LOGINSERVERSHUTDOWN closes the connection as it is shown, which 2.15 confirmed with the real client. A login-screen notice that does not disconnect is adopted if capture or client reverse engineering finds one |
| Schedules on nodes | Definitions live on the panel and replicate to each node their targets sit on. A schedule whose targets are all on one node runs on that node, so it keeps time while the panel link is down and reports its runs when the link returns. A schedule whose targets span nodes runs on the panel, which drives each node's steps and records a node it cannot reach as that step's failure |
| File editor and archives | CodeMirror 6 (MIT), bundled with the dashboard. libarchive (BSD-2-Clause) reads and writes archives, behind the jail and extraction limits File manager in doc/PANEL.md sets. A folder downloads as zip by default, since every operating system opens one without extra tools, and tar.zst is offered beside it |
| SFTP and remote pull | Both are built, opt-in and off by default, with the limits File manager in doc/PANEL.md sets. SFTP uses libssh (LGPL-2.1) in its server mode, since libssh2 has no server side |
| Security keys and passkeys | The panel verifies WebAuthn itself over Botan, for ES256, EdDSA and RS256 credentials, with a small CBOR reader in `src/common` that has a fuzz target of its own. It asks for no attestation, so no attestation format needs trusting, and the page calls the browser's own `navigator.credentials` with no front-end library |
| Two-factor QR codes | qrcode-generator (MIT), a long-maintained encoder with no dependencies, bundled with the dashboard and rendered as SVG in the page, so nothing is fetched from another host |
| Trash and versions | Each writable root keeps them in a hidden `.ambrose/trash` and `.ambrose/versions` folder at its own top, so a delete or a save is a rename on the same volume, and the protected path policy hides both from listings, archives, searches and free-space figures. Defaults, each a live setting: trash kept 30 days and at most 2 GiB per root, versions kept 20 per file and 30 days and at most 512 MiB per root, the oldest going first |
| Database credentials per server | Rotation follows the server: MySQL 8.0.14 and newer adds the new password beside the old one and discards the old after the pool has swapped, and MariaDB, like an older MySQL, gets a second user with the same grants, the pool swaps to it, and the old user is dropped. Realms share one world database by default, and a realm may be given its own, opt-in, such as a test realm with content of its own |
| Maintenance bypass | `Realm.MaintenanceBypassLevel` and `Login.MaintenanceBypassLevel` both default to game master (2), the level that already opens the client's tools under Entering the world |
| World edit exports | Only into `data/sql/custom/db_world`, the tree for local SQL. The panel never writes into a pending folder, which belongs to pull requests |
| Player registration | Offered, off by default through `Panel.Registration.Enable`, on the terms Player accounts in doc/PANEL.md sets: accounts made through AccountMgr's rules at the lowest security level, email verification when SMTP is configured and `Login.RequireVerifiedEmail` says whether it gates sign-in, throttles per address, account and email domain, the operator's own captcha only after repeated attempts, and nothing that tells a stranger whether a name or address exists |
| Patch signing key | Ed25519 through Botan, its private part sealed in the keyring and exportable only by an owner after a step-up check, its public part published with every manifest. Rotation overlaps: for a window the manifest carries signatures from both keys, the new public key is itself signed by the old one, and whatever verifies a manifest accepts either key until the window ends |
| Sequential ramp | Seven steps of one blue, hue 255 in CIELAB, spaced evenly in lightness so more is always more visible: on the dark theme `heat-1` to `heat-7` are #1379AB, #428BBA, #629EC9, #7EB1D8, #99C4E8, #B3D8F7 and #D5EBFF, and on the light theme #5F8BAC, #477CA1, #2A6E96, #016089, #025174, #024260 and #02344C. Every step clears 3:1 against both surfaces of its theme, one hue keeps its order under every kind of color blindness because lightness carries the order, and blue is none of the four meanings. The C-35 candidate was not taken: its lowest value was its most visible step on both themes, and its greens sat beside the teal that means healthy. A magnitude view takes its colors from this ramp and nothing else, which 17.73's gate checks |
| Search | SQLite's FTS5, one index for the log history, the activity log and chat search, kept in a file of its own beside the store so the store stays small. An index row goes when the row it came from leaves its retention, so the index never outlives the caps. The `fts5` feature of the SQLite port is enabled when 17.80 is built |
| Installation file | TOML 1.0, read and written through toml++ (MIT). It holds comments, nested tables and lists of tables, which the file's apps, realms, schedules and grants need, with no ambiguity about types. Every secret in it is a reference into the keyring, never a value |

### World threads, zone data and extracted tables

Settled on 2026-09-25 at the maintainer's direction, recording what 4.01, 4.08, 4.09 and 4.10 built, as the world session gave it.

- One world thread per game server runs the tick. It owns every zone instance, a Map reached through sMapMgr, drains every GameSession's queue and runs each session's WorldUpdate, and duel timers run on the tick. Network threads own the sockets and run only handlers marked InPlace, which touch no world state; a handler that touches world state is marked Queued and runs on the world thread, the split AzerothCore draws between PROCESS_INPLACE and THREADUNSAFE. Database work runs on the pools' async workers, its completion is posted back to the session's network executor, and anything it changes in the world is queued to the world thread from there, as EnterWorld is. The console and the admin API have threads of their own and hand work to the world thread. Updating instances on worker threads, as AzerothCore's MapUpdater does, is the planned scale-out, taken only when a measurement shows the tick saturated; until then nothing touches an instance off the world thread.
- sZoneMgr loads every zone's template, named places and object placements at startup as reload targets. Objects are spawned into an instance when sMapMgr creates it, so an instance nobody enters costs only its rows, and a reload of the placements reaches live instances.
- Object templates are decoded at run time from the user's own install the way the client reads them, through TemplateManifest.xml and then each template file, read on demand, cached per client revision and reloadable, and never stored in a database. Extractors write only what the server queries relationally, such as zone templates, places and placements and the name and creation tables, into the installation's own world database, generated locally from the user's own install and never committed or exported. No separate database is needed, because the world database is already per installation and holds these beside the committed authored rows; where a table mixes generated or imported rows with authored ones, a source column keeps them apart, the rule Server-side content sets for imports.

### Content, SQL and releases

Settled on 2026-09-25 at the maintainer's direction to settle every open decision.

- Tables are normalized, with typed columns and foreign keys, and no column holds JSON the server queries, because MySQL's binary JSON type and MariaDB's JSON, an alias for LONGTEXT with a check, differ in storage, functions and comparison. A client blob the server only stores and returns is a BLOB column. That settles the columns 7.03 and 10.01 asked about.
- A pull request's SQL waits in `data/sql/updates/pending_db_<name>/` as `rev_<unix seconds>_<short-name>.sql`, the moment the author wrote it and a few words for what it holds, so two open pull requests never choose the same name. When it merges, 3.19's job renames each such file to the next free `YYYY_MM_DD_NN.sql` for that UTC date in pending-name order and pushes the rename as a bot commit, which the maintainer approved on 2026-09-16, checked to contain nothing but renames out of pending folders.
- A tool may link the server libraries it reuses, `database` included, as dbimport and the extractors do; the layering order is the only limit.
- Crowns come from two places: a game master's grant, always, and earning in game at rates per source that are live settings, zero by default, so an operator turns earning on and balances it. Nothing is sold for money.
- The tutorial stays in phase 14. Until 14.02 and 14.03 build it, a new wizard starts where `playercreateinfo` says, Ravenwood by default, and those milestones move the start into the tutorial through the same setting.
- The login server checks a client's revision only when `Login.EnforceRevision` is on, off by default because a server supports whichever client an operator has. 16.07 adds DataRevision to the check and makes an empty `Login.AllowedRevision` follow the revision the patch server serves, so publishing a revision moves what the login server accepts in the same step; until it does, an empty list is ignored with a warning, as it is today. 16.07's spec called this setting `Login.RequireRevision`, and now names the one that is built.
