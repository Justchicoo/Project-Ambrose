...
| File | What it is |
|---|---|
| `config.xml` | The `config.xml` the folder already holds, or else the install's own `Bin/config.xml`, or `defaultconfig.xml` from its `Root.wad` under its own root element when the install has none, with `IsFullscreen`, `Resolution`, `WindowedX` and `WindowedY` set to the window asked for and `SilentMetricsURL` emptied, because the client fetches that address while it starts. Everything else, `VersionInfo` included, stays as it is. It is written every run, because the client saves its own over it as it exits, so the window asked for and the empty address hold on every run and the client's other saved settings survive |
