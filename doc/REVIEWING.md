...
- **Ask whose behaviour a check is about, then ask what produced the input.** A check whose subject is the client cannot be earned by a test that builds the message itself: asserting that fields arrive unchanged when the test set them from the same source proves the test copies fields correctly, which is a different and narrower claim. 4.04 wants a real client's MSG_ATTACH echoing what character select gave it, and a server-side handoff test that writes the attach earns 4.06's check instead, honestly. The tell is that nothing in the loop came from outside the code under test. Where several checks need the same client session, say so in the track so one run closes them together rather than one at a time.
...
- **A milestone branch builds itself, and so does main.** `ci_select_legs.py` adds the Linux GCC leg for any branch named `milestone/<id>`, so a milestone pull request compiles without waiting for a label, and for a push to main touching `src/`, `data/`, `cmake/`, `CMakeLists.txt` or `CMakePresets.json`, so a break cannot sit on main until the nightly schedule finds it. A push touching only documentation still costs nothing. Fork runs still need approval for a first-time contributor. Add `ci:windows-msvc-x64` or `ci:all` by hand when the change deserves more.
