...
Phase 1 is built through 1.20: toolchain and CI, utilities, configuration, logging, threading, crypto, the KIWAD reader, the runtime message registry, frames and control messages, the socket layer, and the gameserver, loginserver and patchserver app skeletons. 1.21 (the patch-free launcher) and 1.22 (the session handshake, keepalives and the loginserver bootstrap) are implemented and pass their automated checks. Their real-client checks need the maintainer's own client and follow doc/PATCHING.md. Until they are recorded, work continues on milestones that do not depend on them, starting with the database layer in phase 2. Phase 2 is built: 2.01-2.15 pass their automated checks, and their real-client checks of 2.09, 2.10, 2.14 and 2.15 wait for the maintainer. In phase 3, 3.01, 3.03 and 3.04 are done, 3.02 passes its automated checks with its real-client check waiting for MSG_BADGES, 3.05 passes its automated checks with its real-client check waiting for the character list and creation, 3.06 is done, 3.07 is done, 3.08 is done, 3.09 passes its automated checks with its real-client checks waiting for the maintainer, 3.10 and 3.11 are done, 3.12 passes its unit check with its CharacterCreationConfig.xml checks waiting for 6.10 because the r806919 type dump does not list that file's classes, 3.13 and 3.14 are done, and 3.15 comes next.
