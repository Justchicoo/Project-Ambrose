...
|---|---|---|---|---|
| 5.01 | Template manifest and template store | M | A build and your own install: the client-gated checks read your TemplateManifest.xml and your archives, and `client template <id>` prints any template the way the server reads it, which is the quickest way to check the store against your install | Every client template by id, decoded on demand, cached and reloadable, which every content milestone after it reads. Grow the template manager the world session already built, under this boundary: work in `src/server/game/Entities/ObjectTemplateMgr.{h,cpp}`, new files beside them and their tests; keep `GetPlayer()` and `LoadPlayer()` working as they are; and leave `PlayerObjectBuilder`, `GameSession` and `src/server/game/Entities/Player/` alone, where 5.04 and 5.05 are being built. What the install holds: TemplateManifest.xml has 137,423 entries, 12,807 of which name their file as `|World|Part|path`, meaning the archive World-Part.wad, mostly Recipes-WorldData.wad, so the store needs an archive resolver and not only Root.wad; and a template's behavior list can hold null entries, which the manager already keeps as empty slots that callers rely on. Opened on 2026-09-25 |
| 9.01 | WizCombat protocol surface and stubs | S | A build and your own install: the combat service's message orders come from the client's own definitions, which `client messages --list` prints. The real-client check needs only a test harness that sends a combat message, not a duel | The combat service's 36 messages registered and decoded, so every later combat milestone writes handler bodies instead of wire code. GameSession is still being changed for 4.06, 4.11 and 4.16, so keep to this boundary: the handlers in their own `src/server/game/Handlers/CombatHandler.cpp`; their declarations in one block at the end of `GameSession.h`'s public handler list; their rules in one block after the WIZARD rules in `GameMessageTable.cpp`, with service 51 added to its constructor's list; and nothing in `GameSession.cpp`, `EnterWorld`, `HandleAttach`, `World.cpp` or `WizardHandler.cpp`. Model it on MSG_GETTIMEDACCESSPASSES: a struct in `GameMessages.h` with only the fields a handler reads, `Accept` with the statuses, `InPlace` unless a handler touches world state, and a fixture in `LoginMessageFixtures.h` at the real orders. A message the server does not handle yet is reported once per session, so leaving the rest of the service pending costs a client nothing. Opened on 2026-09-25 |
...
| 4.16 | Being built now by the maintainer's world session |
| 5.04 | Next in the maintainer's world session, which puts the player's stats first |
...
|---|---|---|---|
| 3.28 | MeruneFleuruwu | [#10](https://github.com/Justchicoo/Project-Ambrose/pull/10) | Draft. A per-field evidence report for the 33 layout values and a `--require-derived-layout` guard are in. Left: the derivation itself, which drives the client's own code to place each field, and all five checks; the two naming r801440 and r806919 are run by the maintainer in review |
| 17.23 | MeruneFleuruwu | [#9](https://github.com/Justchicoo/Project-Ambrose/pull/9) | A Compose file, a systemd unit, install scripts and an egg are sent. Left, as the review on #9 lists: the supervisor's own `--install-service` and `--uninstall-service`, which the scripts do not replace; an image that builds, with vcpkg in its build stage; `supervisor.conf.dist` taken from the build output, the players' ports published and the database wired, so `docker compose up` reaches a ready loginserver; a time zone check that proves the system database; the Dockerfile and unit under their own names; and the Compose and time zone checks run and ticked |
| 17.35 | MeruneFleuruwu | [#8](https://github.com/Justchicoo/Project-Ambrose/pull/8) | The page, its routes and the secret masking are built and CI is green. Left: the permission the milestone names, `panel.settings`, which the catalog marks Danger, in place of `settings.read` and `settings.edit`, which are for an app's own settings; a server-side test beside the other panel tests; and the checks it earns ticked with their evidence |
...
| 6.09 | MeruneFleuruwu | before the reset | The sweep is delivered and its two measurable checks are earned, but the oracle the milestone exists for is not: a run collects no unknown properties at all and names nothing for any of the 104 unknown classes, so 520243970 is still unnamed and the draft schema the next milestone feeds on comes out empty |
