...
| 17.23 | MeruneFleuruwu | [#9](https://github.com/Justchicoo/Project-Ambrose/pull/9) | The supervisor registers itself as a Windows service and a systemd unit, the image checks its time zone database while it builds, and Compose serves the players' ports with its database wired. Left, as the second review on #9 lists: the Windows service refusing its own `--service` flag; an image that builds, with `ca-certificates` and a whole vcpkg clone; the egg's done string; tests for the installer; config files only the service can read; and the Compose and time zone checks run and ticked |
