...
|---|---|---|---|
| 3.28 | MeruneFleuruwu | [#10](https://github.com/Justchicoo/Project-Ambrose/pull/10) | A per-field evidence report and a `--require-derived-layout` guard are in. The first review on #10 found that each field is read where `ClientLayout` already puts it, so the derivation can only return the written offset and reports a wrong one as derived, and that its container probe makes every extraction of r806919 fail with six unreadable types. Left: a derivation that drives the client's own code with values the extractor chose, a probe that leaves the process as it found it, and all five checks; the two naming r801440 and r806919 are run by the maintainer in review |
