...
|---|---|---|---|
| 17.23 | MeruneFleuruwu | [#9](https://github.com/Justchicoo/Project-Ambrose/pull/9) | Packaging scripts, the Docker image and Compose file, the egg and the operations notes are sent. Left: the milestone's own deliverable, `supervisor --install-service` and `--uninstall-service` in the supervisor itself, which the install scripts do not replace; the image's tzdata as Time zones in doc/ARCHITECTURE.md settles; the Compose and time zone checks run and ticked; and a description in place of the template. The codestyle failure is not the contributor's: no rule covers a Dockerfile or a systemd unit yet, and the maintainer adds one |
