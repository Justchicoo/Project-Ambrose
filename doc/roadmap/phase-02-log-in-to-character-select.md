...
- [x] Real client: MSG_USER_AUTHEN_V3 (order 27) arrives, is decoded field by field (Version=W.1.610.x, Revision=r806919.Wizard_1_610, Locale, MachineID) and logged. The client driver's enter-world scenario, run 20260925-114135 on 2026-09-25 with the r806919 client, recorded the login server logging `LOGIN MSG_USER_AUTHEN_V3 (7:27) from session 2, 265 bytes` and then `Session 2 sent MSG_USER_AUTHEN_V3: version W.1.610.21, revision r806919.Wizard_1_610, data revision r806919.Wizard_1_610, locale en-US, machine <machine>`, which its step "the server decodes the authentication message field by field" waits for

**Risks**

- The session-offer fields for the packet encryption added in 2021 are not understood; the reference server gets by without them (a reference server's control-message notes). If a newer revision requires them, login is blocked until NET reverse-engineers them.
...
- Formulas studied from a reference server's cryptography and its authentication notes (behavior only, reimplemented from the description)
...
- a reference server's authenticator (behavior only)
