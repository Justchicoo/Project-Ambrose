...
- **Correction.** UNVERIFIED, not wrong: 1.21/phase 1 outcome rely on a '-P 0' client launch flag. No local source documents it (a reference server's documentation documents only '-L 127.0.0.1 12000'). Also unverified: TemplateManifest 137423 entries, 134076 BINd, 42 ItemSetBonusTemplate rows, 16 magic_school_template rows, CombatSigil8Actor's 8 sub-circles, and the traffic.log line citations.
...
- src/common/Utilities/TokenBucket.h: rate limiter (the reference server has one too; behavior only)
...
- None (pure code). Encodings verified against a reference codec's string writer and reader (non-compact path)
...
- Verified against the pinned r806919.Wizard_1_610 install's LatestFileList.xml: CRC matches for sampled files, HeaderSize == KIWAD TOC length for all 3589 type 3/5 WADs, HeaderCRC == CRC(first HeaderSize bytes)
...
- Layout learned from a reference codec's message encoder and a local packet capture tool/sniff.py Framer (behavior reference only)
...
- [x] A DML frame containing two back-to-back DML messages (dmlLen chaining) decodes into 2 messages (a reference decoder supports this; see open question on whether the client sends it)
- [x] Randomized split-point test over 10k generated frames passes under ASan/UBSan

**Risks**

- Long-frame semantics conflict: a reference codec writes the u32 as the message body length with the 0x8000 marker when the body is over 0x777F, while sniff.py computes total = 8 + u32 + 1, which disagrees for DML frames by 8 bytes. Must be confirmed on the wire (NET-6). Both readings are implemented behind `LongFrameLength`, so 1.18 only changes the default
...
- Behavior reference: a reference codec's control-message protocol, a reference server's control service, a reference patch client (SESSION_OFFER_LENGTH = 28)
...
- A reference server treats each recv() as a full packet, which breaks under fragmentation. Do not copy that behavior
...
- src/tools/launcher: the built `launcher` program reads `launcher.conf` or command-line options, starts the user's own client from an Ambrose-owned run folder, and always passes `-L` and `-P 0`. The former `apps/launcher/run-client.ps1` and `run-client.bat` scripts were replaced by milestone 3.25 and are no longer deliverables.
...
- DML frames received before SessionAccept are queued, not dropped (a reference server queues them, which suggests the client can send early)
...
- A reference server suspends heartbeat timeouts during the login-to-game handoff (an explicit halt opcode); a too-strict timeout may drop clients during character select
