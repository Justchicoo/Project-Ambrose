...
- [x] Ordinals equal the name-sorted index: MSG_ALLOWLEAVEPVP=1, MSG_COMBATACTIONS=2, last = MSG_UPDATEDUELTIMER (corrected; the plan said MSG_SETSTATUS)
- [x] MSG_COMBATMOVE and MSG_COMBATPHASEFORSPECTATORS byte round-trip
- [x] MSG_COMBATMOVE rejected when not in world
...
- [x] Unit test: the ordinal of every service-51 message equals its index in the name-sorted list; spot checks MSG_ALLOWLEAVEPVP=1, MSG_COMBATACTIONS=2, MSG_UPDATEDUELTIMER=last
- [x] Unit test: byte round-trip of MSG_COMBATMOVE (MoveType UBYT, SpellSelection UBYT, SpellTarget UINT, TimeLeft INT, ShadowPactTarget INT, SelectedTieredSpellID INT) and MSG_COMBATPHASEFORSPECTATORS (8 STR names), with NOXFER fields skipped
- [x] Unit test: dispatch table rejects MSG_COMBATMOVE from a session not in world
- [x] Real client: nothing visible yet; the server log shows a decoded MSG_COMBATAFK or MSG_COMBATMOVE when forced from a test harness

**Risks**

- Ordinal sorting is inferred from the reference codec generator (a reference codec's message reader sorts with string.CompareOrdinal when no _MsgOrder exists); confirm against a live capture
