...
- My sweep showed mismatches on CharacterElement.m_flags (Bits), AvatarTextureOption.m_textures and TemplateLocation.m_filename until two rules were applied: Bits as strings, and a 31-bit long-length form. a reference codec's own reader uses 15 bits for long strings, which looks like a latent bug. Golden tests must cover strings of 128 bytes or more
- Matrix3x3 serialized width is unconfirmed: a reference codec reads 12 floats, but the name suggests 9. Still unconfirmed after 3.10: no Root.wad file holds a Matrix3x3, Euler, Quaternion or SerializedBuffer value
...
- My sweep showed mismatches on CharacterElement.m_flags (Bits), AvatarTextureOption.m_textures and TemplateLocation.m_filename until two rules were applied: Bits as strings, and a 31-bit long-length form. a reference codec's own reader uses 15 bits for long strings, which looks like a latent bug. Golden tests must cover strings of 128 bytes or more
- Matrix3x3 serialized width is unconfirmed: a reference codec reads 12 floats, but the name suggests 9. Still unconfirmed: no Root.wad file holds one
