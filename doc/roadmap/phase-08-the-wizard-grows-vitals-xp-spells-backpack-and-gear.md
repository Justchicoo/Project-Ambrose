...
|---|---|---|---|
| 8.01 | Live vitals, gold, potions (WIZ-6) | M | 5.05, 6.04 |
| 8.02 | Experience and level-up (WIZ-7) | M | 8.01 |
| 8.03 | School and secondary school (WIZ-8) | M | 8.02, 7.07 |
| 8.04 | SpellMgr: spell templates (CMB-2 spells + WIZ-9 extractor) | M | 5.01, 3.11 |
| 8.05 | Spellbook (WIZ-9) | M | 8.04, 5.05 |
| 8.06 | Item template extractor, core rows (WIZ-10 part 1) | M | 7.01 |
| 8.07 | Item requirements, effects and set bonuses (WIZ-10 part 2) | M | 8.06, 6.10 |
| 8.08 | Backpack: item instances, add/remove/trash (WIZ-11 part 1) | M | 8.06, 6.04 |
...
| 8.13 | Spell training at trainers (WIZ-16) | M | 8.05, 8.02, 7.07 |
...
- USEPOTION handler: needs potion_charge >= 1, restores health and mana by the configured fraction, sends UPDATEPOTIONS, UPDATEHEALTH and UPDATEMANA
- Potion refill timer (config option)
...
- The real potion restore amounts and refill rate are server-side values not in the client files. They are config defaults and need a source.

## 8.02 Experience and level-up (WIZ-7)

**Goal:** XP bar, level-up effect, new base stats.

**Size:** M. **Depends on:** 8.01
...
- src/server/game/Items/ItemMgr (sItemMgr) with reload
...
- src/server/game/Items/Inventory: capacity from ClientWizInventoryBehavior m_numItemsAllowed plus extra space, stacking by m_itemLimit, a global item GID allocator
...
**Size:** M. **Depends on:** 8.05, 8.02, 7.07
...
- src/server/game/Handlers/TrainHandler.cpp: TRAIN handler (validates MobileID in range, the NPC has the trainer service, TrainingIndex in bounds, requirements met, point cost: same school free, off-school costs 1 per the reference, gold cost if any)
