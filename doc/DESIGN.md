...
- **Launcher window** (3.26): under a 44 px chrome bar, the ground fills the frame as a night sky drawn from the surface tokens alone, with the state said once in display type at 44 px low on it, and a bar at the foot carrying the guarantee line, one gold Play, and what this run is: the server, the client revision and the language as values rather than as states, because the launcher has not asked the server anything and must not look as though it had. A realm is a world shard the player picks inside the game; the launcher chooses which Ambrose server to log in to. Its first run shows each setup step with its real numbers and how many of them are done.
